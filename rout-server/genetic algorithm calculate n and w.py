...
def heuristic_function(a, b,ww):
    ...
    heuristic_score =  ((horizontal_inf * ww) + (vertical_inf * (digital-ww)))*distance_aD**n
...
threshold               = 1.25   	# the desired precision of the result
mutation_probability    = 0.1     	# probability of a child mutating
max_mutation            = 0.1*digital   	# 1000 max mutatio
...
fromNode = list(idx.nearest((-3.1391887596691963, 54.601624830066655), 1))[0]
toNode = list(idx.nearest((-3.1171302727307197, 54.60077961329177), 1))[0]
...
with rio_open ('./data/dem.tif') as d:
    ...
    for n in range(61):
        ...
        print(f"n:{n}")
        w = best_match['y']
        w_value.append(best_match['y'])
        fitness_value.append(best_match['fitness'])
